...
describe("Option", () => {
    ...
    describe("cannot mutate", () => {

        it(" options created with Some()", () => {
            const option = Some("foo")

            expect(() => (option[0] as any) = "bar").to.throw()
            expect(() => (option as any).push("bar")).to.throw()
        })

        it("None", () => {
            expect(() => (None[0] as any) = "bar").to.throw()
            expect(() => (None as any).push("bar")).to.throw()
        })
    })
